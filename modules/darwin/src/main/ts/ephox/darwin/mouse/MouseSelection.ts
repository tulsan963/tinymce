    ...
    cursor.on((start) => {
      ...
      findCell(event.target, isRoot).each((finish) => {
        CellSelection.identify(start, finish, isRoot).each((cellSel) => {
          ...
          // This should actually not need to be modified. A single cell in the new Table UI shouldn't
          // actually need to be selected, it just needs the outline + cell button ???
          if (boxes.length > 0 || (boxes.length === 1 && !Compare.eq(start, finish))) {
