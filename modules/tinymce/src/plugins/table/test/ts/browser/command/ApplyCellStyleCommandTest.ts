...
import { afterEach, describe, it, context } from '@ephox/bedrock-client';
...
describe('browser.tinymce.plugins.table.command.ApplyCellStyleCommandTest', () => {
  ...
  context('CellStyleCommand with table_normalize_border', () => {
    const hook = TinyHooks.bddSetup<Editor>({
      plugins: 'table',
      base_url: '/project/tinymce/js/tinymce',
    ...
    it('TINY-7240: Border color must always be accompanied by a border-style: double if border-width is 1px or less', () => {
      const editor = hook.editor();
      editor.setContent(table);
      TinySelections.setCursor(editor, [ 0, 0, 0, 0, 0 ], 0);
      applyCellStyle(editor, { 'border-color': 'red' });
      assertTableCellStructure(editor, { 'border-color': 'red', 'border-style': 'double' });
      applyCellStyle(editor, { 'border-width': '5px' });
      assertTableCellStructure(editor, { 'border-color': 'red', 'border-style': 'solid', 'border-width': '5px' });
    });

    it('TINY-7240: Border-style dashed or dotted must always be accompanied by a border-width of 2px or above', () => {
      const editor = hook.editor();
      editor.setContent(table);
      TinySelections.setCursor(editor, [ 0, 0, 0, 0, 0 ], 0);
      applyCellStyle(editor, { 'border-style': 'dashed' });
      assertTableCellStructure(editor, { 'border-style': 'dashed', 'border-width': '2px' });
    });
    ...
    it('TINY-7240: ', () => {
      ...
      editor.setContent(table);
      TinySelections.setCursor(editor, [ 0, 0, 0, 0, 0 ], 0);
      applyCellStyle(editor, { 'border-style': 'dashed', 'border-width': '5px' });
      assertTableCellStructure(editor, { 'border-style': 'dashed', 'border-width': '5px' });
    });
  });
});
